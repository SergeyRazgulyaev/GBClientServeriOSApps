...
extension GroupsViewController {
    ...
    func loadGroupsFromNetWork() {
        networkService.loadGroupsCodable(token: Session.instance.token) { [weak self] result in
            switch result {
            case let .success(groups):
                for group in groups {
                    self?.groupsNamesFromNetwork.append(group.name)
                    self?.groupsIDFromNetwork.append(group.id)
                    self?.avatarURLFromNetwork.append(group.photo100)
                }
//                print(groups)
                self?.groupsFromNetworkSorting()
...
extension GroupsViewController {
    func groupsFromNetworkSorting() {
        ...
        while tempArray.count > 0 {
            ...
            tempURLArray = []
            tempIDArray = []
//            print(groupedByFirstLetterGroupsID)
        }
    }
}
