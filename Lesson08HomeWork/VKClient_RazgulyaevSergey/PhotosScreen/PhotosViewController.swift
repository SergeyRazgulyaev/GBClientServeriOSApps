...
//                    self?.collectionView.deleteItems(at: insertions.map( {IndexPath(row: $0, section: 0) }))
//                    self?.collectionView.reloadItems(at: insertions.map( {IndexPath(row: $0, section: 0) }))
//                }, completion: nil)
