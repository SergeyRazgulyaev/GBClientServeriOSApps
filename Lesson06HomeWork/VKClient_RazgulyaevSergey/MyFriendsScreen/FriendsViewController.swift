...
class FriendsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        friendsNameIDAvatarArrayСreation()
        friendsFromNetworkSorting()
...
extension FriendsViewController {
    func friendsNameIDAvatarArrayСreation() {
        for user in friendsFromRealm! {
            friendsNamesFromNetwork.append("\(user.firstName) \(user.lastName)")
            friendsIDFromNetwork.append(user.id)
            avatarURLFromNetwork.append(user.photo100)
        }
    }
    
    func friendsFromNetworkSorting() {
        guard friendsNamesFromNetwork.count > 0 else {
            print("Network error")
            return
        }
